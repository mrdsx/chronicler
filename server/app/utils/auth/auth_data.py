...
async def validate_login_data(login_data: AuthSchema_Login) -> None:
    db_user = await get_user_by_email(login_data.email)
    if db_user is None:
...
async def validate_signup_data(signup_data: AuthSchema_SignUp) -> None:
    validate_username(signup_data.username)
    validate_email_address(signup_data.email)
    await validate_user_not_exists(signup_data.username, signup_data.email)
